...
// Main Widget
class TennisTracker extends StatefulWidget {
  @override
  _TennisTrackerState createState() => _TennisTrackerState();
}

class _TennisTrackerState extends State<TennisTracker>
  ...
  int _doubleFaults = 0;
  int _winners = 0;
  int _unforcedErrors = 0;
  int _firstServeSuccess = 0;
  int _firstServeTotal = 0;
  int _breakPointsConverted = 0;
  int _breakPointsTotal = 0;
  int _netPointsWon = 0;
  int _netPointsTotal = 0;
  Timer? _timer;
  ...
  void _initializeHabits() {
    habits = [
      Habit(title: 'Sets', value: '$_sets', icon: Icons.flag),
      Habit(title: 'Aces', value: '$_aces', icon: Icons.star),
      Habit(title: 'Double Faults', value: '$_doubleFaults', icon: Icons.warning),
      Habit(title: 'Winners', value: '$_winners', icon: Icons.emoji_events),
      Habit(title: 'Erreurs', value: '$_unforcedErrors', icon: Icons.error),
      Habit(
          title: '1ère Mise',
          value: '${_firstServeTotal == 0 ? 0 : ((_firstServeSuccess / _firstServeTotal) * 100).toStringAsFixed(1)}%',
          icon: Icons.flag),
      Habit(
          title: 'Break Points',
          value: '$_breakPointsConverted/$_breakPointsTotal',
          icon: Icons.bolt),
      Habit(
          title: 'Filet',
          value: '$_netPointsWon/$_netPointsTotal',
  ...
  void _resetActivity() {
    setState(() {
      ...
      _winners = 0;
      _unforcedErrors = 0;
      _firstServeSuccess = 0;
      _firstServeTotal = 0;
      _breakPointsConverted = 0;
      _breakPointsTotal = 0;
      _netPointsWon = 0;
      _netPointsTotal = 0;
      _initializeHabits();
  ...
  void _incrementSets() async {
    setState(() {
      _sets++;
      habits[0] = Habit(title: 'Sets', value: '$_sets', icon: Icons.flag);

      // Célébration tous les 3 sets
      if (_sets % 3 == 0) {
        _confettiController.play(); // Confettis
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text('Bravo ! Vous avez atteint $_sets sets ! 🎉')),
        );
      }
    });
  }
  ...
  void _incrementDoubleFaults() {
    setState(() {
      _doubleFaults++;
      habits[2] = Habit(title: 'Double Faults', value: '$_doubleFaults', icon: Icons.warning);
    });
  }

  void _incrementWinners() {
    setState(() {
      _winners++;
      habits[3] = Habit(title: 'Winners', value: '$_winners', icon: Icons.emoji_events);
    });
  }

  void _incrementUnforcedErrors() {
    setState(() {
      _unforcedErrors++;
      habits[4] = Habit(title: 'Erreurs', value: '$_unforcedErrors', icon: Icons.error);
    });
  }

  void _incrementFirstServe(bool success) {
    setState(() {
      _firstServeTotal++;
      if (success) _firstServeSuccess++;
      habits[5] = Habit(
          title: '1ère Mise',
          value: '${_firstServeTotal == 0 ? 0 : ((_firstServeSuccess / _firstServeTotal) * 100).toStringAsFixed(1)}%',
          icon: Icons.flag);
  ...
  void _incrementBreakPoint(bool converted) {
    setState(() {
      _breakPointsTotal++;
      if (converted) _breakPointsConverted++;
      habits[6] = Habit(
          title: 'Break Points',
          value: '$_breakPointsConverted/$_breakPointsTotal',
          icon: Icons.bolt);
  ...
  void _incrementNetPoint(bool won) {
    setState(() {
      _netPointsTotal++;
      if (won) _netPointsWon++;
      habits[7] = Habit(
          title: 'Filet',
          value: '$_netPointsWon/$_netPointsTotal',
          icon: Icons.network_wifi);
              ...
              child: Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _buildSummaryRow(Icons.flag, 'Sets', '$_sets', Colors.purple),
                    _buildSummaryRow(Icons.star, 'Aces', '$_aces', Colors.yellow),
                    _buildSummaryRow(Icons.warning, 'Double Faults', '$_doubleFaults', Colors.red),
                    _buildSummaryRow(Icons.emoji_events, 'Winners', '$_winners', Colors.green),
                    _buildSummaryRow(Icons.error, 'Erreurs', '$_unforcedErrors', Colors.orange),
                    _buildSummaryRow(Icons.flag, '1ère Mise', '${_firstServeTotal == 0 ? 0 : ((_firstServeSuccess / _firstServeTotal) * 100).toStringAsFixed(1)}%', Colors.blue),
                    _buildSummaryRow(Icons.bolt, 'Break Points', '$_breakPointsConverted/$_breakPointsTotal', Colors.teal),
                    _buildSummaryRow(Icons.network_wifi, 'Filet', '$_netPointsWon/$_netPointsTotal', Colors.pink),
                  ],
                ),
              ),
            ),
  ...
  Widget _buildMainTitle() {
    return Padding(
      ...
      child: Text(
        'Suivi de votre séance de tennis',
        style: GoogleFonts.bebasNeue(
          fontSize: 32,
  ...
  Widget _buildTennisIconSection() {
    return Container(
