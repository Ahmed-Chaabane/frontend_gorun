...
// Main Widget
class HikingTracker extends StatefulWidget {
  @override
  _HikingTrackerState createState() => _HikingTrackerState();
}

class _HikingTrackerState extends State<HikingTracker>
  ...
  double _elevationGain = 0.0; // Dénivelé en mètres
  ...
  Set<Marker> _markers = {}; // Marqueurs pour les points d'intérêt
  ...
  void _initializeHabits() {
    habits = [
      Habit(title: 'Sessions', value: '$_sessions', icon: Icons.landscape),
      Habit(title: 'Distance', value: '${_distance.toStringAsFixed(2)} km', icon: Icons.map),
      Habit(title: 'Dénivelé', value: '${_elevationGain.toStringAsFixed(0)} m', icon: Icons.terrain),
      ...
      Habit(title: 'Heart Rate', value: '$_heartRate bpm', icon: Icons.favorite), // Optionnel
    ];
  }
  ...
  void _startTracking() {
    ...
    LocationService().getPositionStream().listen((Position position) async {
      setState(() {
        ...
        _updateDistanceAndElevation(position);
        _currentPosition = newPosition;

        // Ajouter un marqueur pour les points d'intérêt (ex : sommets)
        if (_elevationGain > 100 && _markers.isEmpty) {
          _markers.add(
            Marker(
              markerId: const MarkerId("summit"),
              position: newPosition,
              infoWindow: const InfoWindow(title: "Sommet atteint !"),
            ),
          );
        }
      });
  ...
  void _updateDistanceAndElevation(Position position) {
    if (_polylinePoints.length > 1) {
      ...
      for (int i = 1; i < _polylinePoints.length; i++) {
        ...
        // Calcul du dénivelé
        double altitudeDifference = position.altitude - _polylinePoints[i - 1].latitude;
      ...
      setState(() {
        ...
        habits[2] = Habit(title: 'Dénivelé', value: '${_elevationGain.toStringAsFixed(0)} m', icon: Icons.terrain);
      });
    }
  }
  ...
  void _startHeartRateSimulation() {
    Timer.periodic(const Duration(seconds: 5), (timer) {
      setState(() {
        ...
        habits[5] = Habit(title: 'Heart Rate', value: '$_heartRate bpm', icon: Icons.favorite);
      });
    });
  }
              ...
              child: Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _buildSummaryRow(Icons.landscape, 'Sessions', '$_sessions', Colors.purple),
                    _buildSummaryRow(Icons.map, 'Distance', '${_distance.toStringAsFixed(2)} km', Colors.yellow),
                    _buildSummaryRow(Icons.terrain, 'Dénivelé', '${_elevationGain.toStringAsFixed(0)} m', Colors.green),
                    _buildSummaryRow(Icons.timer, 'Durée totale', '${_formatTime(_totalDuration)}', Colors.blue),
                    _buildSummaryRow(Icons.local_fire_department, 'Calories', '$_caloriesBurned kcal', Colors.orange),
                    _buildSummaryRow(Icons.favorite, 'Heart Rate', '$_heartRate bpm', Colors.red), // Optionnel
                  ],
                ),
              ),
            ),
      ...
      child: ClipRRect(
        ...
        child: Container(
          ...
          child: _isLoading
            ...
            markers: _markers, // Marqueurs pour les points d'intérêt
            polylines: {
              Polyline(
